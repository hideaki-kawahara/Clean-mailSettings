...
もう一つのSTARTTLSは、非暗号化通信の途中で暗号化通信に変更が可能なやり方メールのリレーを行います。このため既存のポート番号を変更しなくても良いというメリットがあり、さらに接続先のメールサーバーが暗号化に対応していない場合は暗号化モードに入らず非暗号化のまま通信します、接続してきたメールサーバーが非暗号化のまま通信してきたら続行すれば良いなど多くのメリットがあるためメールサーバーのリレーでよく使われています、、Gmailが使用したため多くのメールサーバーで採用されています。

STARTTLSに対応させるにはメールサーバーの再インストールが必要になるケースもあり、物凄く手間となりますので今回この本では書きません、@<b>{STARTTLSに対応してないメールサーバーだったら対応サーバーに移転を勧めます}。
