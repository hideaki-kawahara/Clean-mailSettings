...
== 送信元メールアドレスはFQDN@<fn>{FQDN}形式で「きれい」
多くのメールサーバにおいて送信元のメールアドレスにドメイン名を使用しないと送信拒否の設定になっている、そしてメールアドレスに使用するドメイン名はFQDN形式でなければなりません。IPアドレス形式のメールアドレスを有効として許可しているメールサーバも存在しますが、仮に送信できたとしても受信側のメールサーバでエラーとして判定されてしまうことも考えられます。受信サーバまで到達したとしてもGmailをはじめとする多くのメールサーバで採用されている@<b>{迷惑メールフィルターがIPアドレス形式のメールアドレスを迷惑メールとして判定してしまいます。}
...
===[column] サーバのホスト名は勝手に名乗れます 
@<b>{サーバのホスト名は勝手に名乗れます！！}

メールのシステム自体がローカルネットワークで使用されている仕組みをそのままインターネット用に拡張して作られため、その当時の仕様がそのまま残っており、そのため@<b>{メールサーバのホスト名は勝手に名乗れます}。メールを送信する側は好き勝手なメールサーバ名を名乗り送信できるので、これが迷惑メールの根本的な原因のひとつとされていることもあり、メールアドレスがFQDNであることは非常に重要な要素といえます。
...
現在のメール送信プロトコルのSMTP@<fn>{SMTP}は2種類あります、最初に考案されたプロトコルは認証に対応しておらず、そのため現在は使われないようになりました。次にOutbound Port25 Blockingにより認証のないメール送信はポート番号@<fn>{ポート番号}からブロックするようになりました。


その後新しいメール送信方法が考案されました。それがPOP before SMTPとSMTP Authenticationです。POP before SMTPはメール受信することであるPOP@<fn>{POP}が認証することを利用しており、メール受信をしたら対象のIPアドレスから一定時間メール送信を許可します。こちらはメールサーバ自体の変更をしなくても可能な方法ですが、仕組みをPOPに頼ることでPOPサーバの負荷がムダにあがってしまうこと、メール受信さえしてしまえば良くて送信については結局何も考慮されていないので、このときに迷惑メールの送信ができてしまう穴があったためしだいに廃れていきました。
...
//image[gmail2][「To自分」の前に出る鍵に斜線が入ったアイコン][scale=1]

自分のメールサーバからGmail宛にメールして鍵に斜線が入ったアイコン表示だったら、STARTTLS非対応と思って間違いないです。
