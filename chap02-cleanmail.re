...
今でこそ、メールはメールアドレスを預かっているメールサーバーから送信するのが当たり前ですが、2005年ぐらいまでは郵便と同じようにメール送信はどこからでも可能でした。しかし迷惑メールの標的となりメール送信に対して制限がかかるようになりました。メールサーバーのドメイン名とEnvelope Fromが一致しないと送信できなくなりました。
...
そして新しいメール送信方法が考案されました、POP before SMTPとSMTP Authenticationです、POP before SMTPはメール受信することであるPOP@<fn>{POP}が認証があることを利用しており、メール受信をしたら対象のIPアドレスから一定時間メール送信を許可します。こちらはメールサーバー自体の変更をしなくても実現可能な方法ですが、仕組みがPOPに依存することでPOPサーバーの負荷が無駄にあがってしまうのと、メール受信さえしてしまえば良くて送信については結局何も考慮されていないということで次第に廃れていきました。
...
//footnote[POP][Post Office Protocolのこと、メールサーバーからメールをダウンロードし、メールはサーバーから削除する、郵便と同じ仕組みの受信方法です。]
...
== メール通信内容における「きれい」
メールサーバー同士の通信は歴史が長くUUCP@<fn>{UUCP}時代までさかのぼります、その頃は当然ながら性善説による世界でありました、このような世界では全ての通信プロトコルは暗号化通信は行っていません。多くの人がインターネットを使うようになってから性善説が通用しなくなりました、現在は多くのプロトコロルが暗号化通信をするようになりました、メールに関しても暗号化通信が導入されていきました。
...
もう一つのSTARTTLSは、非暗号化通信の途中で暗号化通信に変更が可能なやり方でメールの通信などをを行います、このため既存のポート番号を変更しなくても良いというメリットがあります。さらに接続先のメールサーバーが暗号化に対応していない場合は暗号化モードに入らず非暗号化のまま通信します、接続してきたメールサーバーが非暗号化のまま通信してきたら続行すれば良いなど多くのメリットがあるためメールサーバーのリレーでよく使われています、Gmailが使用したため多くのメールサーバーで採用されています。

STARTTLSに対応させるにはメールサーバーの再インストールが必要になるケースもあり、物凄く手間となります。そのため今回この本では書きません、@<b>{STARTTLSに対応してないメールサーバーだったら対応サーバーに移転を勧めます}。
