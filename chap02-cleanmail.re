...
上の4つのことが守れているメールは「きれいなメール」と呼びます、1つでも守られていなければメールは迷惑メールとして判定され、メールを送信しても届かないことになることも考えられる、これらの状況を確認して「きれいなメール」になるため再点検しましょう！
...
== メール通信内容における「きれい」
メールサー間での通信は歴史が長くUUCP@<fn>{UUCP}時代までさかのぼります、その頃のインターネットは性善説による世界でありました、このような世界ではすべての通信プロトコルは暗号化通信を行っていません。多くの人がインターネットを使うようになってから性善説が通用しなくなりました、現在は多くのプロトコロルが暗号化通信をするようになりました、メールに関しても暗号化通信が導入されていきました。SMTPは暗号化通信しておりません、メール本文がそのまま暗号化せずにインターネットを流れ行きます、POPに至ってメール本文だけじゃなくログインIDとパスワードを平文で流してしまいます、とても危険です
