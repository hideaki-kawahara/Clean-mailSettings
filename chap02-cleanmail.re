...
IPアドレス形式なんかしていないと思っていても、ドメイン名の設定にミスがあるとメール送信ができませんし送信先に届きません、例えば@<code>{mail.sample.com}のmailの場合、ホスト側に設定ミスがあると、FQDN形式ではないと判定されるので注意が必要です。@<br>{}
...
@<code>{sample@192.168.0.1}という形ではエラーです。正しくは@<code>{sample@[192.168.0.1]}という形式で書きます。
...
== 送信元における「きれい」
メールを送信する場所、インターネットにおける場所は重要な要素です、たとえば仕事をする上で情報にアクセスするときに制限を加えることがありますが、そのときに使用するのがIPアドレス制限であり正しいところからアクセスしているのかを確認しています、メール送信でも同じでメールアドレス所有者が正しいところからアクセスしているのかが重要です。
...
現在のメールサーバは、Webメールだったり送信時に認証を加えることで送信元の正当性の検証を行っております、ログインまたはメール送信認証さえ通れば送信元が正しいです。
...
そして新しいメール送信方法が考案されました、POP before SMTPとSMTP Authenticationです、POP before SMTPはメール受信することであるPOP@<fn>{POP}が認証があることを利用しており、メール受信をしたら対象のIPアドレスから一定時間メール送信を許可します。こちらはメールサーバ自体の変更をしなくても可能な方法ですが、仕組みがPOPに頼ることでPOPサーバの負荷が無駄にあがってしまうのと、メール受信さえしてしまえば良くて送信については結局何も考慮されていないということで次第に廃れていきました。


SMTP Authenticationはメール送信のプロトコルに対して認証ルールを追加しています、許可されたユーザーのみに対して送信をするように認証する仕組みを追加しています。認証を追加するにあたり既存のプロトコルを拡張するのではなく新たなプロトコルを追加しました、これがサブミッション587番ポートです。現在はSMTP Authenticationの方が使うのが一般的です。
...
== メールサーバにおける「きれい」
次のきれいはメールサーバのきれいです。メールサーバは他のメールサーバと通信しています、メールサーバは近くのメールサーバへリレーをすることでメールを届けています、いくつものメールサーバを経由してメールが届いているのです。このリレー自体は大量のメールをリレーしており、いくつものメールサーバをリレーのリレーに認証を入れることが事実上不可能となっております、ここでは認証のない世界が現在もあります。
...
== メール通信内容における「きれい」
メールサー間での通信は歴史が長くUUCP@<fn>{UUCP}時代までさかのぼります、その頃のインターネットは性善説による世界でありました、このような世界では全ての通信プロトコルは暗号化通信は行っていません。多くの人がインターネットを使うようになってから性善説が通用しなくなりました、現在は多くのプロトコロルが暗号化通信をするようになりました、メールに関しても暗号化通信が導入されていきました。SMTPは暗号化通信しておりません、メール本文がそのまま暗号化せずにインターネットを流れ行きます、POPに至ってメール本文だけじゃなくログインIDとパスワードを平文で流してしまいます、とても危険です


メールの暗号化には2種類の方法が使用されています。TLS@<fn>{TLS}とSTARTTLS@<fn>{STARTTLS}です。
TLSはWebの世界で一般的な方法です、SSLということもありますが、この本ではTLSに統一します、TLSを使ったメール送信ではプロトコル自体が異なってしまうので今まで使ってたポートは使えませんので新しいポート番号を設定します、送信ではSMTP over TLSを使用し、受信も同様に新しいポート番号を設定しPOP over TLSまたはIMAP@<fn>{IMAP} over TLSを使って受信をします。アプリケーションの自動設定やWebメールの利用で、利用者の方で多く使われております。
