...
多くのメールサーバにおいて送信元のメールアドレスにドメイン名を使用しないと送信拒否の設定になっている、そしてメールアドレスに使用するドメイン名はFQDN@<fn>{FQDN}形式でなければなりません。IPアドレス形式のメールアドレスを有効として許可しているメールサーバも存在しますが、仮に送信できたとしても受信側のメールサーバでエラーとして判定されてしまうことも考えられます。受信サーバまで到達したとしてもGmailをはじめとする多くのメールサーバで採用されている@<b>{迷惑メールフィルターがIPアドレス形式のメールアドレスを迷惑メールとして判定してしまいます。}
