...
今現在のメール送信プロトコルSMPT@<fn>{SMTP}は2種類あります、最初に考案されたメール送信のプロトコルは認証に対応していません、そのためこのプロトコルは使われないようになりました。Outbound Port25 Blockingにより認証の無いメール送信はブロックするようになりました。なお25番というのは最初に使われたメールのポート番号です。


そして新しいメール送信方法が考案されました、POP@<fn>{POP} before SMTPとSMTP Authenticationです、POP before SMTPはメール受信することであるPOP(Post Office Protocol)が認証があることを利用しており、メール受信をしたら対象のIPアドレスから一定時間メール送信を許可します。こちらはメールサーバー自体の変更をしなくても実現可能な方法ですが、仕組みがPOPに依存することでPOPサーバーの負荷が無駄にあがってしまうのと、メール受信さえしてしまえば良くて送信については結局何も考慮されていないということで次第に廃れていきました。
...
//footnote[SMTP][Simple Mail Transfer Protocolのことです、。1通のメールを送信するにも何度も通信しサーバーに負荷を与えにくい手順で通信しています。]
//footnote[POP][Post Office Protocolのことです。メールサーバーからメールをダウンロードし、メールはサーバーから削除する、郵便と同じ仕組みの受信方法です。]
...
TLSはWebの世界で一般的な方法です、メール送信では新しいポート番号を設定し、SMTP over TLSでメール送信をし、受信も同様に新しいポート番号を設定しPOP over TLSまたはIMAP@<fn>{IMAP} over TLSで受信をします。この方法は利用者の方で多く使われておりアプリケーションの自動設定の進歩やWebメールの利用でほとんど気付かないまま、このテクノロジーでメールの送受信を行なっております。

もう一つのSTARTTLSは、非暗号化通信の途中で暗号化通信に変更が可能なやり方で既存のポート番号を変更しないメリットがあるため、主にメールサーバーのリレーでよく使われています
...
//footnote[IMAP][Internet Message Access Protocolのことです。POPと異なり、メールをメールサーバ上に保存したまま管理する。Webメールはこの仕組みを使っている。]
