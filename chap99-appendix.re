...
とくにblacklistなどは使用しているドメインにおいて迷惑メール判定されメールが一切届かなくなった確認に便利です、ドメインごと迷惑メール判定されるとエラーメールにならず届かないという現象になるので、このような確認ツールを使うのが良いです。


//raw[|latex|\pagebreak]
== DNSサーバとたわむれよう
DNSサーバを使った遊びです。TXTレコードに原則ルールはありません、ホスト名にプリフィックス名を付けているならほぼ自由に遊びができます。今回使用したドメイン名にも、そんな遊びをやっております、以下にコマンドを紹介するので、ご確認ください。
