...
//raw[|latex|\pagebreak]
== DNSサーバとたわむれよう
DNSサーバを使った遊びです。TXTレコードに原則ルールはありません、ホスト名にプリフィックス名を付けているならほぼ自由に遊びができます。今回使用したドメイン名にも、そんな遊びをやっています、以下にコマンドを紹介します。この本での設定対象はDNSサーバでしたが、DNSサーバはこのような設定もして良いのです。
