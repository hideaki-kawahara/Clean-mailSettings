...
hパラメータについては@<hd>{送信されるときのヘッダー}で述べる。
...
bh=<本文のハッシュ値>;
//}
メール送信するとき電子署名だけじゃなく、さまざまな情報が付加されます、設定には関係ないですが検証時に使う情報がここには含まれています、それがhパラメータがそれです。@<b>{hパラメータに含まれるヘッダーを検証すると同時にメールをリレーするメールサーバは検証を妨害しないように、これらのヘッダーを変更したり修飾しないようにします}。
...
しかし、この基準は将来も守られることはないです、DKIM対応メールサーバが増えればSPFとDKIM両方対応じゃないと迷惑メールと判定する時代になるかもしれません。急にそうなったときに慌てる前にDKIM対応メールサーバに移行することを推奨します。
