...
bh=<本文のハッシュ値>;
//}
メール送信するとき電子署名だけじゃなく、さまざまな情報が付加されます、設定には関係ないですが検証時に使う情報がここには含まれています、それがhパラメーターです。@<b>{hパラメーターに含まれるヘッダーを検証すると同時にメールをリレーするメールサーバは検証を妨害しないように、これらのヘッダーを変更したり修飾しないようにします}。
