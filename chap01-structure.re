...
封筒の外のことはメールヘッダ@<fn>{メールヘッダ}に記載されています、メールヘッダを見ることは普段はしないですが重要な情報が書かれています、メールを出された場所や経由してきたところなどが書かれています、メールを返信するとスレッド化して見ることができるのもメールヘッダに記載された情報を元に実現している機能です。@<b>{SPFやSender IDやDKIMもメールヘッダに記載されいる}ので見たこと無い人は一度見ることをお勧めします。

== Envelope To
ここで一つ疑問が出てきます、メールソフトで見ているTo(送信先)が、封筒の外に書かれている内容は異なっても送ることができるのか？ということです、実際のところTo(送信先)と異なっても送信できます、メールを送信する先は封筒の外に書かれている内容を元にしているのでTo(送信先)は飾りでもフェイクであっても問題ないのです、To(送信先)に大量のメールが列挙されていることがありますが、こちらも同じように、これがそのままの状態で封筒の外に書かれているわけではありません、@<b>{メールサーバがTo(送信先)の数だけ封筒を作ってからメールを送信しています}、封筒はメールサーバが作っているのです、このときの封筒の送信先をEnvelope ToとかRCPT TOということがありますが、この本ではEnvelope Toに統一します。
