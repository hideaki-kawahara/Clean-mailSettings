...
Webメールの画面やメールソフトで見ているときを郵便で例えるなら、封筒に書いてある宛名、差出人はメールヘッダー@<fn>{メールヘッダー}に記載してあります、引き受け郵便局（消印）もメールヘッダーに記載しています。普段皆さんが目にされているのは、メールの中身を取り出したごく一部の内容で、メールソフトではすべての情報を表示しません。郵便物でいうと封筒は見ることなく、中身の便せんだけ見ているような状態です。@<b>{From（送信元）とTo（送信先）は便せんに書かれている内容です}。@<br>{}
...
メールソフトで見ているTo（送信先）が、封筒の外に書かれている内容は異なっても送ることができるのか？ということです。実は、実際にメールを送信する宛先は「封筒の外側」に書かれている内容を元にしているので、封筒の外側と内側でTo（送信先）が異なっていようとも、極端な話内側に書かれているTo（送信先）がフェイクでもメールを送ることはできてしまいます。ときおり、To（送信先）に大量のメールが列挙されていることがありますが、これがそのまま封筒の外に書かれているわけではなく。@<b>{メールサーバがTo（送信先）の数だけ封筒を作ってからメールを送信しています}。
...
//image[header][メールヘッダーの確認方法][scale=0.4,pos=H,border=on]
Gmailでは対象のメールを開いてから右上の3つの点を押下します。メニューに@<b>{「メッセージのソースを表示」}があるので選択し押下します。

確認できるのはパソコンのブラウザからです。Gmailアプリでは確認できません。
