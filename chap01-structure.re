...
== Envelope From
Envelope ToがあればEnvelope Fromも当然存在ます、意識していなければメールアドレスはFrom（送信元）とEnvelope Fromを同じにしますが、メルマガなど大量メール配信の場合などはプログラムで処理をするなどで異なるメールにすることがあります、@<b>{Envelope Fromをメール送信を失敗したときにメールサーバが返信するため送信先メールアドレスとして設定する}ことをします。よく見かける「@<b>{これは送信専用メールです}」とメール本文に書かれているメールはFrom（送信元）とEnvelope Fromを異なるメールアドレスに設定しているのです、このメールに対して返信をするとFrom（送信元）とは異なるメールが表示されます、返信したときに表示される送信先メールアドレスがEnvelope Fromです。@<img>{futo2}にMAIL FROMと記載しているが、この本ではEnvelope Fromに統一します。
