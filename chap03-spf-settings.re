...
== SPFの説明
メールの基本的構造でも記載しましたが、From（送信元）とEnvelope Fromとメールーサーバのドメイン名が異なっても送信できてしまうことから、From（送信元）のメールアドレスの詐称が可能です、実在しないメールアドレスからでも送れます。
...
送信する側の対応が難しいのであれば、受信する側がなんとかすれば良いということで、@<b>{受信メールサーバがEnvelope Fromを検証する仕組みが作られました}、それはDNSサーバを使うことでした。DNSサーバはもともとメールサーバのMXレコード@<fn>{MXレコード}が記載してあるので、メールサーバに関する情報がありました、そのままデータを追加する流れになり新たなレコードを用意されました、これによりDNSサーバを使った検証システムができ上がりました。
...
=== SPFとSender IDの違い
SPFとSender IDは検証するところが異なります、SPFはEnvelope Fromを検証しますが、Sender IDはPRA(Purported Responsible Address)のを参照して検証します。Sender IDはFrom（送信元）まで検証するので、より踏み込んだ検証です。仕様公開当初はライセンス問題@<fn>{ライセンス問題}が存在して多くの人が懸念を表明して、その後ライセンス問題を改修@<fn>{ライセンス問題改修}しましたがメールサーバのアップデートが多くなるや、大企業などでは運用が大変になるなどの理由であまり使われていません、Gmailも使っていません。
...
Resent-Fromについて、メールを再送するときに付加される、メールの再送することがないので、ほとんど使われない。


Senderについて、実際に送信したメールアドレスがFrom（送信元）と異なるときに付加する、たとえばメーリングリストからの配信ではFrom（送信元）と異なるので付加することがある、メルマガなどでは配信エラーの戻り先に設定する事がある。
...
//}
最初のフィールドはバージョン情報です。@<code>{v=spf1}と書きます、他の表記はエラーが発生します。
次のフィールドは定義情報です。空白で区切り複数の定義情報を記載できます。@<code>{+ip4:xxx.xxx.xxx.xxx}と@<code>{~all}が定義情報です。

@<code>{+ip4:xxx.xxx.xxx.xxx} はQualifierとMechanismです。形式は  @<code>{[Qualifier][Mechanism]:[Value]} という感じです。@<br>{}
...
先ほど例を解説すると最初の定義情報は @<code>{+ip4:xxx.xxx.xxx.xxx} なのでValueに指定されたIPアドレスを認証するとなるのでメールを認証するになる。そして次の定義情報は @<code>{~all} になるのですべてマッチするのを認証しないときがあるという意味になるので、最初に指定されたIPアドレスのほかはメール認証をしないときがあるという動きです。
...
ドメイン名を取ったら取りっぱなしの方が多いですがそのまま放置するとドメイン名が汚染される。それを防ぐため、設定をすることが望ましい。ドメイン名が汚染されることを防ぐためにも@<b>{SPFレコードにメール送信をしない設定}をすることが望ましいです。
...
== 判定結果
受信サーバは @<code>{Authentication-Results}というヘッダーに@<code>{spf=}の形式でSPFレコードとEnvelope Fromの検証結果を記録します、記載された情報は次の表です。
...
ここで判定が難しいのは@<b>{softfail}で、SMTP Authenticationを利用していない企業でリモートワークをしているときに企業のネットワークのほかから送信になるケースがあるので、このステータスになることが多いです、それと企業が配信するメルマガも@<b>{softfail}になることが多いです。重要ではないメールであり大量に来るならばブロックしても良いかと思います。


== SPFで得られる効果
送信する側がSPFを設定しないと、メール受信する側はNeutralの判定とするが、これだけではとくに大きな問題にはならない。しかしGmailなどのサービスがNeutral判定を疑わしい判定としており、DKIMの設定もしていないと、はじめにの章でも記載した通り？マークを出して疑わしい表示をしています、いつの日か迷惑メールと判定する可能性があります、SPFの設定は今すぐでもしましょう！
