...
Envelope Fromはメールサーバの方が自動的に補完するので、全て同じドメイン名からしかメール送信できないようにすれば簡単に解決するのですが、それだと利便性に影響が出ます、今となってはSMTP Authenticationなどが一般的になりましたので、それほど利便性に影響しなくなりましたが、大企業などでは外から見えるメールサーバを一つに集約をして運用する事が多く、Envelope Fromを指定してメール送信をすることが難しいです。

送信する側の対応が難しいのであれば、受信する側がなんとかすれば良いということで、@<b>{受信メールサーバがEnvelope Fromを検証する仕組みが作られました}、それはDNSサーバを使うことでした。DNSサーバはもともとメールサーバのMXレコード@<fn>{MXレコード}が記載してあるので、メールサーバに関する情報がありました、そのままデータを追加する流れになり新たなレコードを用意されました、これによりDNSサーバを使った検証システムが出来上がりました。
...
=== SPFとSender IDの違い
SPFとSender IDは検証するところが異なります、SPFはEnvelope Fromを検証しますが、Sender IDはPRA(Purported Responsible Address)のを参照して検証します。Sender IDはFrom(送信元)まで検証するので、より踏み込んだ検証です、仕様公開当初はライセンス問題@<fn>{ライセンス問題}が存在して多くの人が懸念を表明して、その後ライセンス問題を改修@<fn>{ライセンス問題改修}しましたがメールサーバのアップデートが多くなるや、大企業などでは運用が大変になるなどの理由であまり使われいません、Gmailも使っていません。
...
次のフィールドは定義情報です、空白で区切ることで複数の定義情報を記載できます。@<code>{+ip4:xxx.xxx.xxx.xxx}と@<code>{~all}が定義情報です。
...
先ほど例を解説すると 最初の定義情報は @<code>{+ip4:xxx.xxx.xxx.xxx} なのでValueに指定されたIPアドレスを認証するとなるのでメールを認証するになる。そして次の定義情報は @<code>{~all} になるので全てマッチするのを認証しないときがあるという意味になるので、最初に指定されたIPアドレスのほかはメールは認証をしないときがあるという動きです。
...
ドメイン名を取ったらしたら取りっぱなし方が多いですが、そのまま放置するとドメイン名が汚染されることを防ぐためにも@<b>{SPFレコードにメール送信をしない設定}をすることが望ましいです。

迷惑メールを送信する業者が、ドメイン名のメールサーバ名を勝手に名乗り（名乗ることだけは可能）迷惑メールを送信しても、DNSサーバ(DNSサーバは勝手に名乗れない)にメールを送信しない設定をしておくと、受信メールサーバがSPFレコードを参照してFailとして判定してくれるので、そのうち迷惑メール業者がメールサーバ名を勝手に名乗らなくなります。
...
ここで判定が難しいのは@<b>{softfail}で、SMTP Authenticationを利用していない企業でリモートワークをしているときに、企業のネットワークのほかから送信になるケースがあるので、このステータスになることが多いです、それと企業が配信するメルマガも@<b>{softfail}になることが多いです、重要ではないメールであり大量に来るならばブロックしても良いかと思います。
