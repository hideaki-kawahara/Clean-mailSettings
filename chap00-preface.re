...
メールの仕組みと歴史的な経緯もありメールを送信した場所（ドメイン）とメールサーバの正当性を担保することはメールサーバだけでは不可能です。これらの技術はSPF@<fn>{SPF}とSender ID@<fn>{Sender ID}とDKIM@<fn>{DKIM}によって行われます。


本書は、メールの基本的な構造とメール送信で必要不可欠なSPFやSender IDやDKIMの話を、わかりやすく解説した本です。この本を読めばメールのことを深く知り、そしてSPFやDKIMの基礎的なことが身につき送信するメールがスパムメールにされなくなります。
